...
export async function getStats() {
  ...
  // Anchor to end-of-today so floor((endOfToday - e.at)/DAY_MS) groups events
  // by local calendar day instead of rolling 24h windows. Same pattern as
  // buildHourHeatmap in service-worker.js — keeps the bar chart and heatmap
  // consistent and prevents yesterday-evening events from spilling into today.
  ...
  for (const e of log) {
    ...
    const dayOffset = Math.floor((endOfToday.getTime() - e.at) / DAY_MS);
