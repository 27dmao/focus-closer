...
export async function getUsageStats() {
  ...
  // Anchor to end-of-today so floor((endOfToday - e.at)/DAY_MS) buckets entries
  // by local calendar day instead of rolling 24h windows. Same pattern as
  // buildHourHeatmap in service-worker.js and getStats in lib/logger.js — keeps
  // dailyAvgUsd stable regardless of what time of day the dashboard is viewed.
  ...
  for (const e of log) {
    ...
    if (endOfTodayMs - e.at <= 30 * DAY_MS) {
      tokens30d.input += e.input || 0;
      tokens30d.output += e.output || 0;
      tokens30d.cacheRead += e.cacheRead || 0;
      tokens30d.cacheWrite += e.cacheWrite || 0;
    }
    if (endOfTodayMs - e.at <= 7 * DAY_MS) {
      const dayKey = Math.floor((endOfTodayMs - e.at) / DAY_MS);
      perDayUsd.set(dayKey, (perDayUsd.get(dayKey) || 0) + (e.costUsd || 0));
    }
  }

  const dataDays = log.length === 0
    ? 0
    : Math.floor((endOfTodayMs - earliest) / DAY_MS) + 1;
  const daysForAvg = perDayUsd.size || 1;
  let last7Sum = 0;
  for (const v of perDayUsd.values()) last7Sum += v;
  const dailyAvgUsd = perDayUsd.size ? last7Sum / daysForAvg : 0;
