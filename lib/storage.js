...
export async function recordFeedback(verdict, entry) {
  ...
  // Drop any earlier entry for the same videoId (the latest wins).
  const filtered = list.filter((e) => !entry.videoId || e.videoId !== entry.videoId);
  filtered.push({
    at: Date.now(),
    title: entry.title,
